...
sealed class ViewTaskEvent {
    data class UpdateSubTask(val subTask: List<TaskResponse>?) : ViewTaskEvent()
    object Loading : ViewTaskEvent()
    data class Success(val task: CreateTaskResponse?) : ViewTaskEvent()
    object DeleteTask : ViewTaskEvent()
...
data class ViewTaskViewState(
    ...
    fun applyEvent(event: ViewTaskEvent) = when (event) {
        ...
        is ViewTaskEvent.UpdateSubTask -> copy(
            subTask = event.subTask,
            loading = false,
            event = event
        )
        ViewTaskEvent.DeleteTask -> copy(loading = false, event = event)
    }
}
